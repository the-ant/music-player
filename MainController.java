...
public class MainController implements Initializable {
	...
	private void nextSong() {
		...
		case SONG_REPEAT:
			track = tableSongs.getItems().get(ind);
			if (track != null) {
				playSong(track);
			}
			break;
		case SONG_RANDOM:
			...
			ind = rd.nextInt(listSong.size() - 1);
			tableSongs.getSelectionModel().clearAndSelect(ind);
			track = tableSongs.getItems().get(ind);
			if (track != null) {
				playSong(track);
			}
			break;
		default:
			if (listSong.size() == ind + 1) {
				ind = -1;
			}
			tableSongs.getSelectionModel().clearAndSelect(ind + 1);
			track = tableSongs.getItems().get(ind + 1);
			if (track != null) {
				playSong(track);
			}
			break;
	...
	private void previousSong() {
		...
		tableSongs.getSelectionModel().clearAndSelect(ind - 1);
		Track track = tableSongs.getItems().get(ind - 1);
		if (track != null) {
			playSong(track);
		}
	}
